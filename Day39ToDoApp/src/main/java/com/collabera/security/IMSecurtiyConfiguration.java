...
public class IMSecurtiyConfiguration extends WebSecurityConfigurerAdapter {
	...
	throws Exception{	
		...
		//THIS IS KIND OF HARD-CODING USERS INTO THE SYSTEM
		//WE WILL A BE ADDING AN ARRAYLIST TO ACCESS AND ADD USERS FROM AND INTERFACE
//		auth.inMemoryAuthentication().withUser("Oscar")
//		.password(passwordEncoder().encode("123"))
//		.roles("USER","ADMIN");
		
//		auth.inMemoryAuthentication().withUser("bill1993")
//		.password(passwordEncoder().encode("1993"))
//		.roles("USER");
...
//		
	...
	protected void configure(HttpSecurity http) throws Exception{
		...
		.antMatchers("/**") //Must be admin to access User
			.hasAnyRole("ADMIN")
		.and()
			.formLogin()
			.loginPage("/login")
